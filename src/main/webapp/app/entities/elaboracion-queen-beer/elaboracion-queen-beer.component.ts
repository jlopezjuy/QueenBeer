...
export class ElaboracionQueenBeerComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.elaboracionService
                ...
                .subscribe(
                    (res: HttpResponse<IElaboracionQueenBeer[]>) => this.paginateElaboracions(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/elaboracion-queen-beer'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    private paginateElaboracions(data: IElaboracionQueenBeer[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.queryCount = this.totalItems;
        this.elaboracions = data;
    }
