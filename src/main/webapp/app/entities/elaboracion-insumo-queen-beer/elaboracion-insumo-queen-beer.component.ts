...
export class ElaboracionInsumoQueenBeerComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.elaboracionInsumoService
                ...
                .subscribe(
                    (res: HttpResponse<IElaboracionInsumoQueenBeer[]>) => this.paginateElaboracionInsumos(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/elaboracion-insumo-queen-beer'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    private paginateElaboracionInsumos(data: IElaboracionInsumoQueenBeer[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.queryCount = this.totalItems;
        this.elaboracionInsumos = data;
    }
